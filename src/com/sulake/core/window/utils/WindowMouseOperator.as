package com.sulake.core.window.utils
{
   ...
   public class WindowMouseOperator implements IDisposable
   {
      ...
      protected function handler(param1:Event) : void
      {
         ...
         if(_working)
         {
            if(param1.type == Event.ENTER_FRAME)
            {
               ...
               {
                  operate(var_126.mouseX,var_126.mouseY);
      ...
      public function operate(param1:int, param2:int) : void
      {
         _mouse.x = param1;
         _mouse.y = param2;
         getMousePositionRelativeTo(_window,_mouse,var_513);
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(_working)
         {
            if(_window == param1)
            {
               var_126.removeEventListener(MouseEvent.MOUSE_DOWN,handler,false);
               var_126.removeEventListener(MouseEvent.MOUSE_UP,handler,false);
               var_126.removeEventListener(Event.ENTER_FRAME,handler);
               if(true)
               {
      ...
      public function begin(param1:IWindow, param2:uint = 0) : IWindow
      {
         ...
         {
            var_126.addEventListener(MouseEvent.MOUSE_DOWN,handler,false);
            var_126.addEventListener(MouseEvent.MOUSE_UP,handler,false);
            var_126.addEventListener(Event.ENTER_FRAME,handler);
            _mouse.x = var_126.mouseX;
            _mouse.y = var_126.mouseY;
