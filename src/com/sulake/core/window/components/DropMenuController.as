package com.sulake.core.window.components
{
   ...
   public class DropMenuController extends InteractiveController implements IDropMenuWindow
   {
      ...
      private function getSubMenu() : DropMenuController
      {
         ...
         {
            var_134 = context.create(name + "::submenu","",WindowType.const_393,var_89,0 | 0,_loc1_,subMenuEventProc,null,0,null,[WindowController.EXCLUDE_CHILD_TAG]) as DropMenuController;
         }
         else
         {
      ...
      private function subMenuEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         else if(param1.type == WindowEvent.const_801)
         {
            if(param2 == var_134)
            {
               closeMenuView();
      ...
      protected function populateMenuItemContainer(param1:Array, param2:DropMenuController, param3:Function) : void
      {
         ...
         while(_loc9_ < _loc5_)
         {
            _loc6_ = context.create(name + "::menu_item_" + _loc9_,param1[_loc9_],WindowType.const_429,var_89,0 | 0 | 0 | 0 | 0,null,param3,null,_loc9_,null,["_EXCLUDE"]) as DropMenuItemController;
         ...
         var _loc10_:IWindow = context.create(name + "::padding","",WindowType.const_298,var_89,0 | 0 | 0 | 0,new Rectangle(0,0,1,2),null,null,0,null,["_EXCLUDE"]);
         ...
         if(var_447 > -1 && _loc5_)
         {
            _loc4_.getListItemAt(var_447).setStateFlag(WindowState.const_45,true);
         }
      }
      ...
      protected function openMenuView() : void
      {
         if(!var_526)
         {
            if(open())
            {
               if(false)
               {
                  populateMenuItemContainer(var_448,getSubMenu(),subMenuEventProc);
                  var_526 = true;
               }
            }
         }
      ...
      protected function closeMenuView() : void
      {
         if(close())
         {
            ...
            var_526 = false;
            getTitleText().text = var_447 < var_448.length && var_447 > -1 ? "null" : caption;
         }
      }
   }
}
