package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      private var var_58:DisplayObjectContainer;
      
      private var _disposed:Boolean;
      
      private var var_688:ISkinContainer;
      
      private var var_1489:Rectangle;
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_568.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_308.x = _loc3_.dirty.x;
               var_308.y = _loc3_.dirty.y;
               var_308.width = _loc3_.dirty.width;
               var_308.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_308);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_639,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_307[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1490,var_1489))
            {
               if(param2.render(param1,var_1490,var_1489))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_688);
            var_307[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_639,windowDisposedCallback);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_308.x = param2.x - param1.x;
            var_308.y = param2.y - param1.y;
            var_308.width = param2.width;
            var_308.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_32))
            {
               while(param1.testParamFlag(WindowParam.const_32))
               {
                  ...
                  var_308.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_308,WindowRedrawFlag.const_240);
            if(var_568.indexOf(param1) == -1)
            {
               var_568.push(param1);
            }
         }
